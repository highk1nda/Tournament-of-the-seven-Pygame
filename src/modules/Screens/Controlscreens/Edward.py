...
class Edward():
    ...
    def update(self):
        ...
        if self.Intro.is_clicked((mx, my), self.click):
            ...
            return textscr(con.display_surface, con.clock, "Edward").run()
        if self.Ending.is_clicked((mx, my), self.click) and con.storyEdwardComplete:
            self.click = False
            con.select_sound.play()
            return textscr(con.display_surface, con.clock, "Edward", True).run()
        if self.Ending.is_clicked((mx, my), self.click) and not con.storyEdwardComplete:
            self.click = False
